...
const RANK_COLORS: Record<string, string> = {
  'S+': 'text-tertiary', S: 'text-tertiary', A: 'text-secondary-fixed',
  B: 'text-primary', C: 'text-on-surface', D: 'text-outline', F: 'text-valo-red',
};
...
export default function ResultsPage({ result, onNavigate }: ResultsPageProps) {
  ...
  if (!result) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <button onClick={() => onNavigate('home')} className="text-primary font-mono hover:underline">Go Home</button>
      </div>
    );
  }
  ...
  const handleSave = () => {
    ...
    setNameError('');
    saveToLeaderboard(playerName, result);
    setSaved(true);
  };
      ...
      <div className="relative z-10 w-full max-w-3xl flex flex-col gap-6 animate-fade-in">
        ...
        <div className="text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-heading font-black text-transparent bg-clip-text bg-gradient-to-r from-primary via-secondary-fixed to-tertiary uppercase">
            {isPerfect ? 'Perfect!' : result.accuracy >= 80 ? 'Great Job!' : result.accuracy >= 60 ? 'Not Bad!' : 'Try Again!'}
          </h1>
          <p className="text-on-surface-variant font-body mt-2">{game?.name} · {mode?.name}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* Big score */}
          <div className="rounded-xl p-8 border border-primary/20 bg-surface-container flex flex-col items-center text-center"
            style={{ background: 'rgba(19,27,46,0.8)', boxShadow: '0 0 30px rgba(210,187,255,0.1)' }}>
            <span className="text-xs font-mono text-outline uppercase tracking-widest mb-2">Final Score</span>
            <div className="text-7xl font-heading font-black text-primary">{result.score.toLocaleString()}</div>
            {result.timeBonus > 0 && (
              <span className="text-xs font-mono text-secondary-fixed mt-1 flex items-center gap-1">
                <span className="material-symbols-outlined text-xs">bolt</span>+{result.timeBonus} time bonus
              </span>
            )}
            <div className="mt-3 flex items-center gap-2">
              <span className="text-xs font-mono text-on-surface-variant">Rank</span>
              <span className={`text-4xl font-heading font-black ${RANK_COLORS[rank]}`}>{rank}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: 'Accuracy', value: `${result.accuracy}%`, icon: 'my_location', color: result.accuracy >= 80 ? 'text-secondary-fixed' : result.accuracy >= 60 ? 'text-primary' : 'text-valo-red' },
              { label: 'Correct', value: `${result.correctAnswers}/${result.totalQuestions}`, icon: 'check_circle', color: 'text-on-surface' },
              { label: 'Max Streak', value: `${result.maxStreak}x`, icon: 'local_fire_department', color: result.maxStreak >= 5 ? 'text-valo-red' : 'text-tertiary' },
              { label: 'Time Bonus', value: `+${result.timeBonus}`, icon: 'bolt', color: 'text-tertiary' },
            ].map(stat => (
              <div key={stat.label} className="rounded-xl p-4 border border-outline-variant/20 bg-surface-container flex flex-col gap-1">
                <div className="flex items-center gap-1">
                  <span className={`material-symbols-outlined text-sm ${stat.color}`} style={{ fontVariationSettings: "'FILL' 1" }}>{stat.icon}</span>
                  <span className="text-xs font-mono text-outline uppercase">{stat.label}</span>
                </div>
        ...
        <div className="rounded-xl p-6 border border-primary/20 bg-surface-container">
          ...
          <p className="text-on-surface-variant text-sm font-body mb-4">Enter your name to add your score to the leaderboard.</p>
          ...
          {saved ? (
            <div className="flex items-center gap-3 p-4 rounded-lg bg-secondary-fixed/10 border border-secondary-fixed/30 animate-fade-in">
              <span className="material-symbols-outlined text-secondary-fixed text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
              <div>
                <p className="font-heading font-semibold text-secondary-fixed">Score saved!</p>
                <p className="text-xs font-mono text-on-surface-variant">Your score has been added to the leaderboard.</p>
              </div>
            </div>
          ) : (
            <div className="flex gap-3">
              <div className="flex-1">
                ...
                />
                {nameError && <p className="text-valo-red text-xs font-mono mt-1">{nameError}</p>}
              </div>
              <button
                onClick={handleSave}
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button
            ...
            style={{ background: 'linear-gradient(135deg, #7c3aed, #00dbe7)', boxShadow: '0 0 15px rgba(210,187,255,0.3)' }}
          >
